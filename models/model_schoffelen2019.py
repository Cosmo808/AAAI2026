    ...
    def __init__(self, args: Any):
        ...
        if args.model == 'simplecnn':
            self.backbone = SimpleConv(
                in_channels=273, out_channels=320, num_layers=1,
            ...
            self.final = nn.Sequential(
                nn.Conv1d(273, 640, kernel_size=1, stride=1),
                # nn.BatchNorm1d(2 * 128),
                nn.GroupNorm(num_groups=16, num_channels=640),
                nn.Dropout(0.5),
                nn.GELU(),
                nn.ConvTranspose1d(640, 1024, kernel_size=1, stride=1),
            )

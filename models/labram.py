...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1000, patch_size=200, in_chans=1, out_chans=8, num_classes=1, embed_dim=200, depth=12,
                 num_heads=10, mlp_ratio=4., qkv_bias=False, qk_norm=partial(nn.LayerNorm, eps=1e-6), qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0.1, norm_layer=partial(nn.LayerNorm, eps=1e-6), init_values=0.1,
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        pos_embed_used = self.pos_embed[:, input_chans] if input_chans is not None else self.pos_embed[:, :(n + 1)]
        if self.pos_embed is not None:

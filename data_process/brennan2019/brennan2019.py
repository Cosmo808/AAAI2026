...
def process_subject(base_path, idx, length=None, sample_t=None):
    ...
    if length is not None:
        ...
        for i in range(0, len(timestamp), 3):
        ...
        # stop_timestamp = start_timestamp + length
        # while stop_timestamp <= timestamp[-1] + duration[-1]:
        #     index = (timestamp > stop_timestamp).argmax()
        #     index -= 1
        #     stop_timestamp = timestamp[index] + duration[index]

        #     _, rep = wav2vec(sound_event=sound_events, start=start_timestamp, stop=(start_timestamp + length))
        #     if rep is None:
        #         start_timestamp = timestamp[index + 1]
        #         stop_timestamp = start_timestamp + length
        #         continue
        #     s = int(start_timestamp * sample_rate)
        #     sample_num = int(sample_rate * length)
        #     eeg = torch.tensor(data[:, s:(s + sample_num)])
        #     if eeg.shape[-1] == 0:
        #         continue
        #     eeg_seg.append(eeg)
        #     rep = rep.permute(0, 2, 1)
        #     rep = F.interpolate(rep, size=sample_num)
        #     speech_rep.append(rep.squeeze(0))

        #     start_stop.append([start_timestamp, stop_timestamp])
        #     start_index = np.where(timestamp == start_timestamp)[0][0]
        #     speech_seg.append(' '.join(word[start_index:index + 1]))
        ...
        for i, (s, d, s_seg) in enumerate(zip(timestamp, duration, speech)):
            ...
            if 0.7 * sample_t <= d < 2 * sample_t:
                ...
                if concat_s is None:
                    concat_s, concat_d, concat_s_seg = s, d, s_seg
                while i + 1 < len(timestamp):
                    i += 1
                    s_next, d_next, s_seg_next = timestamp[i], duration[i], speech[i]
                    concat_d += d_next
                    concat_s_seg += ' ' + s_seg_next

                    if concat_d < 2 * sample_t:
                        _, rep = wav2vec(sound_event=sound_events, start=concat_s, stop=(concat_s + concat_d))
                        if rep is None:
                            break
                        else:
                            rep = rep.permute(0, 2, 1)
                            rep = F.interpolate(rep, size=resample_length)
                            speech_rep.append(rep.squeeze(0).detach().cpu())
                            start_stop.append([concat_s, concat_s + concat_d])
                            speech_seg.append(concat_s_seg)
                            slice_data = torch.tensor(data[:, int(concat_s * sample_rate):int((concat_s + concat_d) * sample_rate)])
                            resample_data = resample(slice_data, sample_num=resample_length)
                            eeg_seg.append(resample_data.cpu())
                    else:
                        break
        ...
        # uncontinuous concat
        added_s, added_d, added_s_seg = [], [], []
        for i in range(len(duration)):
            for j in range(i + 1, len(duration)):
                if 0.7 * sample_t < duration[i] + duration[j] < 2 * sample_t and abs(timestamp[i] - timestamp[j]) < 10 * sample_t:
                    added_d.append((duration[i], duration[j]))
                    added_s.append((timestamp[i], timestamp[j]))
                    added_s_seg.append((speech[i], speech[j]))
        for (s1, s2), (d1, d2), (s_seg1, s_seg2) in zip(added_s, added_d, added_s_seg):
            _, rep1 = wav2vec(sound_event=sound_events, start=s1, stop=(s1 + d1))
            _, rep2 = wav2vec(sound_event=sound_events, start=s2, stop=(s2 + d2))
            if rep1 is not None and rep2 is not None:
                rep1 = rep1.permute(0, 2, 1)
                rep2 = rep2.permute(0, 2, 1)
                slice_data1 = torch.tensor(data[:, int(s1 * sample_rate):int((s1 + d1) * sample_rate)])
                slice_data2 = torch.tensor(data[:, int(s2 * sample_rate):int((s2 + d2) * sample_rate)])

                # normal
                rep = torch.cat([rep1, rep2], dim=-1)
                rep = F.interpolate(rep, size=resample_length)
                speech_rep.append(rep.squeeze(0).detach().cpu())
                start_stop.append([s1, d1 + d2])
                slice_data = torch.cat([slice_data1, slice_data2], dim=-1)
                resample_data = resample(slice_data, sample_num=resample_length)
                eeg_seg.append(resample_data.cpu())
                speech_seg.append(s_seg1 + ' ' + s_seg2)

                # # inverse
                # rep = torch.cat([rep2, rep1], dim=-1)
                # rep = F.interpolate(rep, size=resample_length)
                # speech_rep.append(rep.squeeze(0).detach().cpu())
                # start_stop.append([s2, d2 + d1])
                # slice_data = torch.cat([slice_data2, slice_data1], dim=-1)
                # resample_data = resample(slice_data, sample_num=resample_length)
                # eeg_seg.append(resample_data.cpu())
                # speech_seg.append(s_seg2 + ' ' + s_seg1)
    return start_stop, speech_seg, speech_rep, eeg_seg


def data_split(subject_idxes, method: tp.Tuple[str, int]):
    method, fold = method[0], method[1]
    if method == 'half':
        num_subject = len(subject_idxes) // 2
        train_idxes = subject_idxes[:num_subject]
        test_idxes = subject_idxes[num_subject:]
        print('train idxes:', train_idxes)
        print('test idxes:', test_idxes)
        if fold == 0:
            return {'train': train_idxes, 'test': test_idxes}
        elif fold == 1:
            return {'train': test_idxes, 'test': train_idxes}
    elif method == '5fold':
        num_subjects = len(subject_idxes)
        fold_size = num_subjects // 5
        folds = [subject_idxes[i:i + fold_size] for i in range(0, num_subjects, fold_size)]
        if len(folds) > 5:
            folds[-2].extend(folds[-1])
            folds = folds[:-1]
        test_fold = folds[fold]
        train_folds = [f for i, f in enumerate(folds) if i != fold]
        train_idxes = [item for sublist in train_folds for item in sublist]
        test_idxes = [item for item in test_fold]
        print('train idxes:', train_idxes)
        print('test idxes:', test_idxes)
        return {'train': train_idxes, 'test': test_idxes}
...
def save_datasets(base_path, split_method=None, length=None, sample_t=None):
    ...
    # idxes_splits = data_split(subject_idxes, method=split_method)
    # for split, idxes in idxes_splits.items():
    #     subject_id_list, start_stop_list, speech_seg_list, speech_rep_list, eeg_seg_list = [], [], [], [], []
    #     for i, idx in enumerate(idxes):
    #         print(f"Processing subject {i + 1}/{len(idxes)} for {split} split")
    #         start_stop, speech_seg, speech_rep, eeg_seg = process_subject(base_path, idx, length)
    ...
    #     datasets = brennan2019(subject_id_list, start_stop_list, speech_seg_list, speech_rep_list, eeg_seg_list)
    #     save_path = os.path.join(base_path, fr"data_splits_l{length}\{split_method[0]}{split_method[1]}_{split}.pt")
    #     os.makedirs(os.path.dirname(save_path), exist_ok=True)
    #     torch.save(datasets, f=save_path)
    #     del datasets, start_stop_list, speech_seg_list, speech_rep_list, eeg_seg_list
    ...
    if split_method[0] == 'half':
        ...
        train = brennan2019(subject_id_list[:num], start_stop_list[:num], speech_seg_list[:num], speech_rep_list[:num], eeg_seg_list[:num])
        test = brennan2019(subject_id_list[num:], start_stop_list[num:], speech_seg_list[num:], speech_rep_list[num:], eeg_seg_list[num:])
        if split_method[1] == 0:
        ...
        test_dataset = brennan2019(test_subject_id, test_start_stop, test_speech_seg, test_speech_rep, test_eeg_seg)
        ...
        train_dataset = brennan2019(train_subject_id, train_start_stop, train_speech_seg, train_speech_rep, train_eeg_seg)
...
if __name__ == "__main__":
    ...
    # args.length = 5
    save_datasets(base_path=path, split_method=split_method, length=args.length, sample_t=8)
    # save_datasets(base_path=path, split_method=split_method, length=length)
